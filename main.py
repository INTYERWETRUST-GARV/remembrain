...
class RemembrainApp:
    ...
    def _build_gui(self):
        ...
        self.topic_btn = tk.Button(
            self.panel_frame, text="Update Last Topic",
            font=self.font_status, bg="#0f8b8d", fg="white",
            activebackground="#0c6f70", activeforeground="white",
            relief=tk.FLAT, padx=10, pady=8, cursor="hand2",
            command=self._on_update_topic_click
        )
        self.topic_btn.pack(padx=15, fill=tk.X)
        ...
        self.root.bind("<V>", lambda e: self._on_save_unknown_voice_click())
        self.root.bind("<t>", lambda e: self._on_update_topic_click())
        self.root.bind("<T>", lambda e: self._on_update_topic_click())
    ...
    def _update_card(self, person_key, person_info):
        ...
        last_topic_summary = self._get_last_topic_summary(person_info)
        ...
        self.info_labels["last_topic"].config(text=last_topic_summary)
        ...
        if last_topic_summary and last_topic_summary != "No previous topic summary recorded.":
            reminder += f" Last time, you talked about: {last_topic_summary}."
    ...
    def _update_card_unknown(self):
        ...
        self.info_labels["last_topic"].config(text="—")
    ...
    def _summarize_topic_text(self, raw_text):
        """Create a short summary from rough discussion points without storing full text."""
        cleaned = re.sub(r"\s+", " ", (raw_text or "").strip())
        if not cleaned:
            return ""

        parts = re.split(r"[.!?]", cleaned)
        first_sentence = next((p.strip() for p in parts if p.strip()), cleaned)
        summary = first_sentence

        if len(summary) > TOPIC_SUMMARY_MAX_CHARS:
            summary = summary[:TOPIC_SUMMARY_MAX_CHARS].rstrip(" ,.;:") + "..."

        return summary

    def _get_last_topic_summary(self, person_info):
        """Return stored short topic summary if available."""
        summary = (person_info or {}).get("last_topic_summary", "")
        summary = summary.strip() if isinstance(summary, str) else ""
        if not summary:
            return "No previous topic summary recorded."
        return summary
    ...
    def _on_update_topic_click(self):
        """Store a short summary of the last conversation topic for current person."""
        if not self.current_person_key:
            self.status_label.config(
                text="● Recognize someone first, then update last topic",
                fg="#f0a500",
            )
            return

        name = self.people_db.get(self.current_person_key, {}).get("name", "this person")
        raw_topic = simpledialog.askstring(
            "Update Last Topic",
            f"What did you talk about with {name}?\n(Only a brief summary will be stored.)",
            parent=self.root,
        )
        if raw_topic is None:
            return

        summary = self._summarize_topic_text(raw_topic)
        if not summary:
            self.status_label.config(
                text="● Topic update skipped (empty input)",
                fg="#f0a500",
            )
            return

        person = self.people_db.get(self.current_person_key, {})
        person["last_topic_summary"] = summary
        person["last_topic_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M")
        self._save_people_database()

        self.info_labels["last_topic"].config(text=summary)
        self.status_label.config(
            text=f"● Saved short topic summary for {name}",
            fg="#53bf9d",
        )
